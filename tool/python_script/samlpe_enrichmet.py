...
outfile = open(output_path + "/temp.txt", "w")
...
inFile = open(output_path + "/temp.txt")
outFile = open(output_path + "/phenotype_report.txt", "w")
data = inFile.readlines()
count = 0
while count < len(data):
...
while count < len(data):
    if data[count].split(':')[0] == "Label":
        ...
        while temp < len(data) and len(data[temp]) != 0:
            if is_minimum(data[temp].split(':')[0], output_path + "/temp.txt", float(data[temp + 1].split(':')[1])) == True:
...
os.system("rm -rf " + output_path + "/temp.txt")
outFile.close()

# output to different files
x = open(output_path + "/" + "phenotype_report.txt")
data = x.readlines()
count = 0
while count < len(data):
    if data[count].split(':')[0] == "Label":
        aList = []
        for x in blocks_list_data:
            aList.append("\n")

        filename = data[count].split(':')[1][1:-1]
        print filename
        sample_outFile = open(output_path + "/phenotype_" +  filename, "w")
        temp = count
        temp = temp + 1
        while temp < len(data) and len(data[temp]) != 1:
            number = int(data[temp].split(':')[0][2:])
            aList[number] = data[temp].split(':')[1][1:]
            temp = temp + 2
        print aList
        for line in aList:
            sample_outFile.writelines(line)
        count = count + 1
    else:
        count = count + 1
